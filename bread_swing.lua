...
status_messages = {
    ["poisoned"] = {"%c* *You feel[^%.]+sick\.%c*",
                    "%c* *You are no longer poisoned\.%c*"},
...
function bread_swing()
    if not c_persist.swing_slot
    or you.turns() == 0 and not brstate.set_slot then
        find_swing_slot()
  end
    ...
    brstate.wielding = false
    if not brstate.dir_x
    or not safe_swing_pos(brstate.dir_x, brstate.dir_y) then
